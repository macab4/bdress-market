...
// prenda activa hace más de 14 días que nunca la destacaron — un único
// recordatorio por prenda (boost_reminder_sent_at, migración
// 20260831000000), nunca se repite aunque siga sin destacar.
...
export async function GET(request: Request) {
  ...
  const cutoff = new Date(Date.now() - MIN_LISTING_AGE_DAYS * 24 * 60 * 60 * 1000).toISOString()
  ...
  const { data: candidates, error } = await admin
    ...
    .is('boost_reminder_sent_at', null)
